...
import { CloseButton, LockIcon } from './icons';
...
// Outer wrapper keeps AnimatePresence mounted so the exit animation can play
// when `tile` becomes null.
export default function TileDetailModal(props: TileDetailModalProps) {
  return (
    <AnimatePresence>
      {props.tile && <TileDetailContent key={props.tile.id} {...props} tile={props.tile} />}
    </AnimatePresence>
  );
}

function TileDetailContent({ room, myPlayerId, tile, tiles, property, players, allProperties = [], onClose }: TileDetailModalProps & { tile: Tile }) {
  const TILES = tiles;
  ...
  const setAdvantage = tile.set && owner
    ? (ownsFullSet(TILES, allProperties, tile.set, owner.id) ? SET_ADVANTAGES[tile.set] : null)
    : null;

  // Progress for non-owner or viewer: show cities owned count
  const ownerSetProgress = tile.set && owner
    ? { owned: getSetOwnerCount(TILES, allProperties, tile.set, owner.id), total: SET_SIZES[tile.set] ?? 2 }
    : null;
  ...
  const mySetOwned = tile.set && myPlayerId ? getSetOwnerCount(TILES, allProperties, tile.set, myPlayerId) : 0;
  ...
  return (
    <>
      <motion.div
        style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'oklch(0.12 0.02 260 / 0.55)', backdropFilter: 'blur(5px)', WebkitBackdropFilter: 'blur(5px)' }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={onClose}
      ...
      >
        {/* Set color band */}
        {setColor && (
          <div style={{ height: 3, background: setColor }} />
        )}
            ...
            <div style={{ padding: '16px 16px' }}>
              ...
              {/* Monopoly bonus — active when the owner holds the full set */}
              {tile.set && owner && SET_ADVANTAGES[tile.set] && (
                <SetBonusPanel
                  setColor={setColor}
                  active={!!setAdvantage}
                  description={SET_ADVANTAGES[tile.set]}
              ...
              {/* Tax */}
              {tile.type === 'tax' && (
                <div style={{ padding: '10px 14px', borderRadius: 'var(--r-md)', textAlign: 'center', background: 'var(--danger-soft)', border: '1px solid oklch(0.71 0.155 25 / 0.25)' }}>
              ...
              {isMine && room && myPlayerId && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {!isMyTurn && (
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'var(--danger)' }}>
                        Not your turn
                      </span>
                    )}
                  </div>
                  ...
                  {error && (
                    <ErrorNote style={{ fontSize: 9.5, padding: '6px 10px', marginBottom: 10, textAlign: 'center' }}>
                      {error}
                    </ErrorNote>
                  )}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                        ...
                        {!hasMonopoly && (
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 3 }}>
                            <LockIcon size={8} /> Monopoly Required
                          </span>
                        )}
                      </div>
                    ...
                    {isMortgaged ? (
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: '1px solid oklch(0.82 0.11 85 / 0.35)',
